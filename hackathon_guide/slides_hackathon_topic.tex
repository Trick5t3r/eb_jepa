...
\begin{frame}{Pipeline — one engine: examples to reuse, tracks to complete}
  \begin{center}\small\itshape One generic, config-driven JEPA engine. We ship complete examples \& a per-track baseline; each track only fills a small \texttt{\#TODO} surface.\end{center}
  \vspace{0.12cm}
  \begin{center}
    \resizebox{0.95\textwidth}{!}{%
    \begin{tikzpicture}[
      nd/.style={draw=primary, fill=primary!10, rounded corners=3pt, text width=2.3cm,
                 minimum height=0.9cm, align=center, font=\scriptsize\bfseries, text=primary, inner sep=3pt},
      nd2/.style={draw=secondary, fill=secondary!10, rounded corners=3pt, text width=2.3cm,
                  minimum height=0.9cm, align=center, font=\scriptsize\bfseries, text=secondary!80!black, inner sep=3pt},
      ar/.style={->, thick, color=primary!60}]
      \node[nd]  (data) at (0,0)    {Track data\\\scriptsize windows};
      \node[nd2] (enc)  at (3.2,0)  {Encoder};
      \node[nd2] (ssl)  at (6.4,0)  {SSL JEPA};
      \node[nd]  (pr)   at (9.8,0)  {Frozen probe\\ vs baselines};
      \draw[ar] (data)--(enc); \draw[ar] (enc)--(ssl); \draw[ar] (ssl)--(pr);
      \node[font=\scriptsize, text=primary!70] at (3.2,-1.0) {conv $\mid$ transformer};
      \node[font=\scriptsize, text=primary!70] at (6.4,-1.0) {predictive $\mid$ vicreg};
      \node[font=\scriptsize, text=primary!70, align=center] at (9.8,-1.05) {random $\mid$ supervised\\$\mid$ DLinear\,\ldots};
    \end{tikzpicture}}
  \end{center}
  \vspace{0.12cm}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{qbox}{secondary}{piprow}
        \cardtitle{secondary}{Provided — study \& reuse (not tracks)}
        \footnotesize
        \qitem{\texttt{image\_jepa} — SSL on CIFAR-10}
        \qitem{\texttt{video\_jepa} — Moving-MNIST prediction}
        \qitem{\texttt{ac\_video\_jepa} — Two Rooms: world model \& planning}
        \qitem{\itshape exploratory: \textbf{the maze} — A\textsuperscript{*}-free hierarchical navigation}
      \end{qbox}
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{qbox}{primary}{piprow}
        \cardtitle{primary}{Tracks — a baseline $+$ your \texttt{\#TODO}}
        \footnotesize
        \qitem{each track ships a runnable \textbf{baseline}}
        \qitem{\texttt{build\_pretrain\_dataset} — SSL windows}
        \qitem{\texttt{build\_eval\_dataset} — $(x,y)$ pairs}
        \qitem{\texttt{task\_type}, \texttt{in\_channels} \;{\itshape (opt: encoder / metric)}}
      \end{qbox}
    \end{column}
  \end{columns}
  \vspace{0.1cm}
  \begin{center}\scriptsize
    \texttt{cp template\_track.py my\_track.py} ~$\rightarrow$~ fill \texttt{\#TODO} ~$\rightarrow$~
    \texttt{config.yaml} ~$\rightarrow$~ \texttt{cli.pretrain} / \texttt{cli.evaluate}
  \end{center}
\end{frame}

\begin{frame}{One engine, swappable modules — all config-driven}
  \begin{center}\small\itshape A track only declares its data \& task; every module below is provided and picked in \texttt{config.yaml}.\end{center}
  \vspace{0.25cm}
  \begin{center}\footnotesize
  \begin{tabular}{@{}lll@{}}
    \toprule
    \textbf{Module} & \textbf{Options (config-selectable)} & \textbf{Role} \\
    \midrule
    Data interface          & track loaders — SSL windows / $(x,y)$ pairs               & feed the engine \\
    Encoder                 & Conv1d \,$\mid$\, Transformer                              & observations $\to$ latents \\
    SSL objective           & predictive JEPA \,$\mid$\, VICReg                          & self-supervised pretraining \\
    Predictor / world model & RNN (GRU) \,$\mid$\, Transformer                           & action-conditioned next-latent \\
    Planning                & MPPI \,$\mid$\, CEM                                        & goal-reaching by latent energy min. \\
    Hierarchy {\itshape(opt.)} & subgoal predictor                                      & A\textsuperscript{*}-free hierarchical nav \\
    Evaluation              & frozen probe vs baselines (random / supervised / DLinear) & score the representation \\
    Trainer                 & AMP loop \,+\, metrics harness                            & shared, untouched \\
    \bottomrule
  \end{tabular}
  \end{center}
  \vspace{0.3cm}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{qbox}{primary}{artrow}
        \cardtitle{primary}{Pick \emph{predictive} when\ldots}
        \footnotesize
        \qitem{there is temporal / sequence structure to predict}
        \qitem{an action or control channel is available}
        \qitem{$\Rightarrow$ video, planning, time series}
      \end{qbox}
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{qbox}{secondary}{artrow}
        \cardtitle{secondary}{Pick \emph{vicreg} when\ldots}
        \footnotesize
        \qitem{you have strong, meaning-preserving augmentations}
        \qitem{no natural ``next step'' to predict}
        \qitem{$\Rightarrow$ EEG, audio, point clouds}
      \end{qbox}
    \end{column}
  \end{columns}
\end{frame}
