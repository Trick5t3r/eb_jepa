...
\begin{frame}{Pipeline — one engine: swappable modules}
  ...
  \vspace{0.15cm}
  % repo screenshots in place of the tikz — root / eb_jepa / examples (tune height as needed)
  \begin{center}
    \includegraphics[height=3cm]{figures/screenshot_home}\hfill
    \includegraphics[height=3cm]{figures/screenshot_utils}\hfill
    \includegraphics[height=3cm]{figures/screenshot_example}
  \end{center}
  \vspace{0.2cm}
  \begin{columns}[T]
...
\begin{frame}{Pipeline — reuse, complete, explore}
  \begin{center}\small\itshape Reuse a complete example · complete a track by filling a tiny \texttt{\#TODO} surface · then explore.\end{center}
  \vspace{0.3cm}
  {\footnotesize
  \begin{itemize}\itemsep0.5em
    \item \textbf{Reuse} — runnable examples: \texttt{image\_jepa} · \texttt{video\_jepa} · \texttt{ac\_video\_jepa/two\_rooms} \;(+ explore \texttt{maze}).
    \item \textbf{Complete} — 6 tracks (audio · EEG · point cloud · FinTime · LTSF · Gray-Scott): harness provided, you fill \textbf{3 \texttt{\#TODO}} — \texttt{build\_encoder} · \texttt{build\_ssl}/\texttt{build\_jepa} · \texttt{probe}.
    \item \textbf{Explore} — start from the baseline, then make it yours.
  \end{itemize}}
  % >>> add a repo screenshot below (e.g. the examples/ tree, or a track README #TODO) <<<
  \vfill
\end{frame}
