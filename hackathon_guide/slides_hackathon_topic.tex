...
\begin{frame}{Pipeline — one engine: swappable modules}
  ...
  \begin{center}\footnotesize
    \textbf{Swap} the engine in \texttt{eb\_jepa/} (\texttt{jepa·architectures·losses·planning}) \;·\; \textbf{wire} it per example in \texttt{examples/<x>/} (\texttt{main·eval·config.yaml}) — all config-driven.
  \end{center}
  % >>> add a repo screenshot below (e.g. the eb\_jepa/ tree, or architectures.py) <<<
  \vfill
\end{frame}
