...
\begin{frame}{Pipeline — a modular, config-driven JEPA scaffold}
  \begin{center}\small\itshape One generic engine; each track only fills a small \texttt{\#TODO} surface.\end{center}
  \vspace{0.12cm}
  \begin{center}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{qbox}{secondary}{piprow}
        \cardtitle{secondary}{Provided — don't touch}
        \footnotesize
        \qitem{Encoders: Conv1d \,/\, Transformer}
        \qitem{Objectives: predictive JEPA \,/\, VICReg}
        \qitem{Predictor: RNN (GRU) \,/\, Transformer}
        \qitem{AMP trainer \,+\, probe harness \,+\, metrics}
      \end{qbox}
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{qbox}{primary}{piprow}
        \cardtitle{primary}{You fill — the \texttt{\#TODO} surface}
        \footnotesize
        \qitem{\texttt{build\_pretrain\_dataset} — SSL windows}
        \qitem{\texttt{build\_eval\_dataset} — $(x,y)$ pairs}
        \qitem{\texttt{task\_type}, \texttt{in\_channels}}
        \qitem{\itshape optional: encoder / metric / augment}
      \end{qbox}
    \end{column}
  \end{columns}
  \vspace{0.1cm}
...
\begin{frame}{One Pipeline, Many Tracks}
  \begin{center}\small\itshape Same engine for every modality — a track $=$ data loaders $+$ a task declaration.\end{center}
  \vspace{0.25cm}
  \begin{center}\footnotesize
  \begin{tabular}{@{}lllc@{}}
    \toprule
    \textbf{Track} & \textbf{SSL objective} & \textbf{Downstream} & \textbf{You write} \\
    \midrule
    ...
    Maze / Two-rooms (gen.)   & predictive (action-cond.)& planning / probe     & loader \\
    Time series (fin.\,/\,LTSF) & predictive             & forecasting / regr.  & loader $+$ baseline \\
    \bottomrule
  \end{tabular}
  \end{center}
  \vspace{0.3cm}
