...
$DSETS/the_well/gray_scott_reaction_diffusion  # The Well / gray_scott (Track 4)
$DSETS/speech_commands_v2                       # Speech Commands v2  (Track 2)
$DSETS/modelnet40/modelnet40_ply_hdf5_2048      # ModelNet40          (Track 6)
$DSETS/Neuro                                    # TUAB / TUEV EEG     (Track 1)
\end{lstlisting}
...
\end{lstlisting}
Planning uses MPPI by default (\code{cfgs/planning\_mppi.yaml}); point
\code{eval.plan\_cfg\_path} at \code{cfgs/planning\_cem.yaml} for CEM.
...
(Track~1) is the worked example of all four.

% ===========================================================================
\subsection{Group A: Carry the JEPA recipe to a new modality}

\begin{trackbox}{Track 1 (Flagship) --- EEG abnormality detection on TUAB \hfill \diffmed{}/\diffhard{}}
\textbf{The idea.} Pretrain a JEPA on raw clinical EEG, then linear-probe it to
classify recordings as normal vs abnormal. This mirrors the EEG foundation model
LaBraM~\citep{LaBraM} but replaces its
VQ tokenizer and Fourier-spectrum reconstruction with a pure joint-embedding
objective: \emph{predict patch representations in latent space}, never the noisy
raw signal. That structural choice is the point. LaBraM had to reconstruct the
spectrum because raw-EEG reconstruction would not converge; a JEPA sidesteps the
problem entirely, since the target encoder is free to discard unpredictable
artifacts.

\smallskip
\textbf{Data.} TUH Abnormal EEG Corpus (TUAB), a labeled subset of the TUH EEG
Corpus~\citep{TUEG}: $\sim$2{,}717 training and 276 evaluation recordings, binary normal/abnormal,
in EDF format. Pretrain on a small slice of the larger unlabeled TUH corpus (tens
to $\sim$100 hours is plenty for 24h; do not try to ingest the full corpus).
Standardize to a common $\sim$21--23 channel montage at 200\,Hz, bandpass
$0.1$--$75$\,Hz, 60\,Hz notch, z-score per channel.

\smallskip
\textbf{Beyond TUAB.} The same TUH family gives a natural difficulty ladder for the
\emph{same} pretrained encoder: \textbf{TUEV} (TUH EEG Events) is a harder 6-class
event-classification task (e.g. spike/sharp-wave, GPED, PLED, eye-movement,
artifact, background), and the broader unlabeled \textbf{TUEG} corpus is the
pretraining pool. A strong project pretrains once and probes both TUAB
(binary, easy) and TUEV (6-class, hard) to show where the recipe holds and where it
strains.

\smallskip
\textbf{More TUH corpora available locally (downloaded for this hackathon).} Two
additional labeled task corpora extend the difficulty ladder for the \emph{same}
frozen encoder, beyond TUAB/TUEV: \textbf{TUAR} (TUH EEG Artifact Corpus,
\code{TUAR\_RAW\_DATA}) --- per-segment \emph{artifact} detection/typing (eye movement,
muscle, chewing, shiver, electrode, instrument artifacts on the \code{01\_tcp\_ar}
montage), a clinically useful and noise-robust probe; and \textbf{TUEP} (TUH EEG
Epilepsy Corpus, \code{TUEP\_RAW\_DATA}) --- \emph{patient-level} epilepsy vs
no-epilepsy classification (\code{00\_epilepsy} / \code{01\_no\_epilepsy}, multi-session
per subject, \code{01\_tcp\_ar} and \code{02\_tcp\_le} montages), which forces
subject-disjoint splits and tests whether the representation transfers to a coarse,
patient-level label rather than a per-window one. Both are raw EDF (same preprocessing
as TUAB: common montage, 200\,Hz, bandpass, per-channel z-score) and reuse the existing
linear-probe harness.
...
\begin{itemize}[leftmargin=1.4em,itemsep=1pt,topsep=2pt]
  \item \emph{Temporal} (video-JEPA style, \cref{sec:video}): cut each channel into
    ...
    window's representation across all channels. This is next-frame prediction with
    frames $=$ time windows.
  \item \emph{Masked} (image-JEPA style, \cref{sec:image}): build the
    channel$\times$time patch grid, mask a fraction of patches, predict their
    representations from the visible ones; use EEG augmentations (channel dropout,
    amplitude scaling, time jitter) for the two-view variant.
\end{itemize}

\smallskip
\textbf{Files to touch.} A new \code{Dataset} under \file{eb\_jepa/datasets/} that
loads EDF (use \code{MNE-Python} or \code{braindecode}'s \code{TUHAbnormal}) and
yields \code{[B, C, T, 1, 1]} patch tensors; a small conv-patch or 1D-conv encoder
in \file{architectures.py}; the GRU \code{RNNPredictor} for the temporal framing; a
linear probe modeled on \file{image\_jepa/eval.py}.

\smallskip
\textbf{Metric.} Balanced Accuracy on the 276-recording eval set (also report
AUROC). Context: supervised CNNs reach $\sim$0.79 Balanced Accuracy, BIOT
$\sim$0.796, fine-tuned LaBraM-Base $\sim$0.81. A frozen JEPA \emph{linear probe}
clearing $\sim$0.78 would be a genuinely strong hackathon result.

\smallskip
\textbf{Gotchas.} Variable montages across recordings (restrict to the common 10--20
set for simplicity, or use per-electrode embeddings); per-channel normalization is
mandatory; class-aware metric because the eval split is mildly imbalanced. Report
metrics at the recording level, not the raw 10-second-window level.

\smallskip
\textbf{poc:} TUAB frozen linear-probe \textbf{AUROC 0.877} (patient-disjoint split,
near supervised SOTA); on the harder TUEV 6-class event task, $\sim$0.68 AUROC.
TUAR (artifact) and TUEP (patient-level epilepsy) are downloaded and ready as further
probes for the same encoder.
\emph{Data:} \code{/lustre/work/pdl17890/udl806719/datasets/Neuro} ---
\code{TUAB-TUEV/} (TUAB+TUEV) and \code{TUAB-TUEV-PREPROCESSED/}
(\code{TUAR\_RAW\_DATA}, \code{TUEP\_RAW\_DATA}, \code{TUEV\_PREPROCESSED}) ~|~ HF
\href{https://huggingface.co/datasets/DBClab/TUAB-TUEV-PREPROCESSED}{DBClab/TUAB-TUEV-PREPROCESSED}.
\end{trackbox}

\begin{trackbox}{Track 2 --- Audio / speech representation JEPA \hfill \diffmed{}}
...
\begin{trackbox}{Track 3 --- Wearable biosignals: ECG / IMU / PPG \hfill \diffeasy{}/\diffmed{}}
...
\begin{trackbox}{Track 4 --- Latent dynamics of physical fields (The Well) \hfill \diffmed{}/\diffhard{}}
...
\paragraph{Forecasting as action-conditioned prediction (Tracks 5--7).} The
three time-series tracks below share one mapping onto the codebase. The
AC-video-JEPA (\cref{sec:acvideo}) is a latent dynamics model: given a state and
an \emph{action}, it predicts the next state's representation. For a time series
the ``action'' is whatever exogenous or control channel drives the
system---actuator commands, manipulated set-points, the operating regime, known
future covariates, or order-flow events---and ``predict the future'' is simply
forecasting the next state in latent space. Anomaly detection then falls out for
...
the rollout forecast or the per-step energy. Two gotchas apply to all three: the
...
\begin{trackbox}{Track 5 --- Scientific multivariate time-series forecasting \hfill \diffmed{}/\diffhard{}}
...
Track~4 (The Well) and ERA5/WeatherBench.
...
\begin{trackbox}{Track 6 --- Financial time series: limit order books \& forecasting \hfill \diffmed{}/\diffhard{}}
...
daily rates)~\citep{LSTNet} sits in the same LTSF suite as Track~5, and the Monash
...
series with learnable dynamics (the LTSF / Exchange-Rate suites of Track~5, or FI-2010 LOB
above).
...
\begin{trackbox}{Track 7 --- Action-conditioned anomaly prediction \& predictive maintenance \hfill \diffmed{}/\diffhard{}}
...
\begin{trackbox}{Track 8 --- 3D point clouds / LiDAR \hfill \diffhard{}}
...
\begin{trackbox}{Track 9 --- Learned cost / value for planning \hfill \diffmed{}/\diffhard{}}
...
\begin{trackbox}{Track 10 --- Hierarchical / multi-timescale world model \hfill \diffhard{}}
...
\begin{trackbox}{Track 11 --- Stress-test the recipe under factors of variation \hfill \diffhard{}}
...
\begin{trackbox}{Track 12 --- Does intuitive physics emerge? \hfill \diffmed{}}
