...
Throughout, \code{monospaced paths} point at real files in the repository, and
boxes flag \textbf{tips}, \textbf{gotchas}, and \textbf{notes} worth pausing on.
...
\file{examples/launch\_sbatch.py} point at the original authors' cluster. The
organizers will give you the correct \code{--partition}/\code{--account} (or a
patched launcher) for the hackathon B200 nodes. Do not burn an hour debugging a
queue rejection; ask. See \cref{sec:logistics}.
\end{gotchabox}
...
\begin{trackbox}{Track 5 --- Scientific / financial multivariate time series \hfill \diffmed{}}
\textbf{Goal.} JEPA on noisy multivariate series (sensor networks, traffic, market
data) with a downstream forecasting or regime-classification probe. The interesting
...
\textbf{Files:} time-series dataset; 1D encoder; \code{RNNPredictor}.
\textbf{Metric:} downstream probe vs a supervised baseline on the same split.
\end{trackbox}

\begin{trackbox}{Track 6 --- 3D point clouds / LiDAR \hfill \diffhard{}}
...
\begin{trackbox}{Track 7 --- Learned cost / value for planning \hfill \diffmed{}/\diffhard{}}
...
\begin{trackbox}{Track 8 --- Hierarchical / multi-timescale world model \hfill \diffhard{}}
...
\begin{trackbox}{Track 9 --- Stress-test the recipe under factors of variation \hfill \diffhard{}}
...
\begin{trackbox}{Track 10 --- Does intuitive physics emerge? \hfill \diffmed{}}
...
\begin{enumerate}[leftmargin=1.6em,itemsep=1pt,topsep=2pt]
  \item \textbf{A code artifact}: a fork or branch with your changes, runnable from
    a single documented command, plus the config(s) you used.
  \item \textbf{A short report} (1--2 pages or a slide deck): the hypothesis, the
    setup, the result with at least one figure or table, and one paragraph on what
    you learned about JEPAs (especially any collapse you fought).
  \item \textbf{A 3-minute demo} to the room: ideally a live or recorded artifact, a
    probe-accuracy curve, a planning rollout GIF, a reconstruction-free latent
    prediction, whatever shows your result.
\end{enumerate}
